...
fn p1(input: &str) -> i32 {
    input
        ...
        .filter_map(|line| {
            ...
            let number = format!("{}{}", first, last).parse::<i32>().ok();
            number
        })
        .sum::<i32>()
}
    ...
    input
        ...
        .filter_map(|line| {
            let v = word_number
                ...
                .flat_map(|&word| {
                    if line.contains(word) {
                        ...
                        indexes
                            ...
                            .map(move |index| {
                                (
                                    word_number.iter().position(|&r| r == word).unwrap() + 1,
                                    index,
                                )
                            })
                            .collect::<Vec<_>>()
                ...
                .chain(line.chars().filter(|c| c.is_ascii_digit()).flat_map(|c| {
                    let indexes = find_all_indexes(line, &c.to_string());
                    indexes
                        .into_iter()
                        .map(move |index| (c.to_digit(10).unwrap() as usize, index))
                        .collect::<Vec<_>>()
                }))
            ...
            let number = format!("{}{}", first.0, last.0).parse::<i32>().ok();
            number
        })
        .sum::<i32>()
}
